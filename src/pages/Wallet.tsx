    ...
    <div className="p-4 lg:p-8 space-y-12 animate-in fade-in duration-700 pb-32">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h1 className="text-4xl font-bold text-foreground tracking-tighter uppercase">My Wallet</h1>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-blue-600/20 to-black border border-blue-500/20 p-8 rounded-[5px] relative overflow-hidden group">
          ...
          <div className="flex items-end gap-3">
            <span className="text-5xl font-black text-foreground tracking-tighter">0.00</span>
            <span className="text-xl font-bold text-muted-foreground mb-1">TON</span>
          </div>
        ...
        <div className="bg-gradient-to-br from-purple-600/20 to-black border border-purple-500/20 p-8 rounded-[5px] relative overflow-hidden group">
          ...
          <div className="flex items-end justify-between">
            <div className="flex items-end gap-3">
              <span className="text-5xl font-black text-foreground tracking-tighter">{userProfile.jamBalance || '0'}</span>
              <span className="text-xl font-bold text-muted-foreground mb-1">JAM</span>
            </div>
        ...
        <div className={`border p-8 rounded-[5px] relative overflow-hidden group transition-all ${userProfile.isPremium ? 'bg-gradient-to-br from-amber-600/20 to-black border-amber-500/20' : 'bg-muted/50 border-border'}`}>
          <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
          ...
          <div className="flex items-center gap-3">
            <span className="text-4xl font-black text-foreground tracking-tighter uppercase">
              {userProfile.isPremium ? 'Premium' : 'Standard'}
            </span>
            {userProfile.isPremium && <CheckCircle2 className="h-6 w-6 text-amber-500" />}
          </div>
          <p className="text-[9px] font-bold text-muted-foreground/50 uppercase tracking-widest mt-6">
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {jamPackages.map(pkg => (
            <div key={pkg.id} className={`relative p-8 rounded-[5px] border transition-all hover:scale-[1.02] cursor-pointer group ${pkg.popular ? 'bg-blue-600/5 border-blue-500/30' : 'bg-muted/50 border-border'}`}>
              {pkg.popular && (
              ...
              <div className="flex justify-between items-start mb-6">
                ...
                <Coins className={`h-6 w-6 ${pkg.popular ? 'text-blue-500' : 'text-muted-foreground/50'}`} />
              </div>
              
              <div className="flex items-end gap-2 mb-8">
                <span className="text-4xl font-black text-foreground tracking-tighter">{pkg.amount}</span>
                <span className="text-lg font-bold text-muted-foreground mb-1 uppercase">JAM</span>
              </div>
              
              <button 
                onClick={() => handlePurchaseJAM(pkg.price, pkg.amount)}
                disabled={isProcessing}
                className={`w-full py-4 rounded-[5px] text-[10px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-3 ${pkg.popular ? 'bg-blue-600 text-foreground shadow-xl shadow-blue-500/20' : 'bg-muted/50 text-muted-foreground/80 hover:bg-muted'}`}
              >
      ...
      {!userProfile.isPremium && (
        <section className="relative rounded-[5px] overflow-hidden bg-gradient-to-br from-amber-600/20 to-black border border-amber-500/20 p-8 lg:p-16">
          ...
          <div className="relative z-10 max-w-2xl space-y-6">
            ...
            <h2 className="text-4xl lg:text-6xl font-black uppercase tracking-tighter leading-[0.9] text-foreground">
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={i} className="flex items-center gap-3 text-sm text-muted-foreground/80 font-medium">
            ...
            <div className="pt-6">
              <button 
                ...
                disabled={isProcessing}
                className="px-12 py-5 bg-amber-600 hover:bg-amber-500 text-foreground font-bold uppercase tracking-widest rounded-[5px] transition-all shadow-xl shadow-amber-600/20 flex items-center gap-4"
              >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-foreground/[0.02] border-b border-border/50">
                  <th className="px-6 py-4 text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Type</th>
                  <th className="px-6 py-4 text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Asset / Protocol</th>
