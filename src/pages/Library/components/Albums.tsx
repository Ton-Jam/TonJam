...
export const Albums: React.FC<AlbumsProps> = ({ albums, layout = 'list' }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <LayoutGroup id="albums-layout-group">
        <motion.div
          layout
          transition={{ duration: 0.25, ease: [0.25, 1, 0.5, 1] }}
          className={layout === 'grid' ? "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4" : "space-y-2"}
        >
          {sortedAlbums.map((album) => (
            <motion.div
              ...
              key={album.id}
              transition={{ duration: 0.25, ease: [0.25, 1, 0.5, 1] }}
              whileHover={layout === 'grid' ? { scale: 1.02 } : { x: 4 }}
              className={
                layout === 'grid'
                  ? "bg-white/[0.02] dark:bg-white/[0.02] bg-black/[0.02] border border-black/5 dark:border-white/5 p-3 rounded-[10px] flex flex-col justify-between group"
                  : "bg-white/[0.02] dark:bg-white/[0.02] bg-black/[0.02] border border-black/5 dark:border-white/5 p-3 rounded-[10px] flex items-center justify-between group gap-4 text-left"
              }
            >
              {layout === 'grid' ? (
                <>
                  <motion.div 
                    ...
                    className="relative aspect-square w-full rounded-[10px] overflow-hidden mb-3 bg-slate-800"
                  >
                  ...
                  <div className="space-y-1">
                    <motion.h4 
                      ...
                      className="text-xs font-extrabold text-foreground leading-snug truncate group-hover:text-primary transition-colors"
                    >
                      {album.title}
                    </motion.h4>
                ...
                <>
                  <div className="flex items-center gap-3 min-w-0">
                    <motion.div 
                      layoutId={`album-cover-${album.id}`}
                      className="relative w-12 h-12 rounded-lg overflow-hidden bg-slate-800 shrink-0"
                    >
                      <img src={album.coverUrl} alt={album.title} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      {album.isDownloaded && (
                    ...
                    <div className="min-w-0 text-left">
                      <motion.h4 
                        layoutId={`album-title-${album.id}`}
                        className="text-xs font-extrabold text-foreground truncate group-hover:text-primary transition-colors"
                      >
                        {album.title}
                      </motion.h4>
                      <p className="text-[10px] text-muted-foreground truncate font-semibold">
