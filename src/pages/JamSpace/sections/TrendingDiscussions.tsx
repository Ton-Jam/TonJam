  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-wider">Dynamic Node Ledger</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {discussions.map((disc, idx) => (
          <motion.div
            key={disc.id}
            className="bg-white/[0.03] hover:bg-white/[0.05] p-3.5 sm:p-4 rounded-2xl flex flex-col justify-between border-none transition-colors"
            whileHover={{ y: -2 }}
          >
            <div className="space-y-2">
              <span className="text-xs font-bold text-[#00B4D8]">{disc.hashtag}</span>
