          ...
          {step === 1 && (
            <form onSubmit={handleProfileSubmit} className="space-y-6 animate-in fade-in slide-in-from-right duration-500 relative z-10">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Artist Name</label>
                  <input type="text" value={profileData.name} onChange={e => setProfileData({...profileData, name: e.target.value})} className="w-full bg-muted/50 -white/10 rounded-[10px] px-4 py-3 text-sm font-bold text-foreground outline-none focus:-blue-500 transition-colors" placeholder="Enter your stage name" required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Handle (@)</label>
                  <input type="text" value={profileData.handle} onChange={e => setProfileData({...profileData, handle: e.target.value})} className="w-full bg-muted/50 -white/10 rounded-[10px] px-4 py-3 text-sm font-bold text-foreground outline-none focus:-blue-500 transition-colors" placeholder="@username" required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Bio</label>
                  <textarea value={profileData.bio} onChange={e => setProfileData({...profileData, bio: e.target.value})} className="w-full bg-muted/50 -white/10 rounded-[10px] px-4 py-3 text-sm font-bold text-foreground outline-none focus:-blue-500 transition-colors h-32 resize-none" placeholder="Tell your story..." />
                </div>
              </div>
              <div className="pt-4">
          ...
          {step === 2 && (
            <form onSubmit={handleTrackSubmit} className="space-y-6 animate-in fade-in slide-in-from-right duration-500 relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Track Title</label>
                    <input type="text" value={trackData.title} onChange={e => setTrackData({...trackData, title: e.target.value})} className="w-full bg-muted/50 -white/10 rounded-[10px] px-4 py-3 text-sm font-bold text-foreground outline-none focus:-blue-500 transition-colors" placeholder="e.g. Midnight Protocol" required />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Genre</label>
                    <select value={trackData.genre} onChange={e => setTrackData({...trackData, genre: e.target.value})} className="w-full bg-muted/50 -white/10 rounded-[10px] px-4 py-3 text-sm font-bold text-foreground outline-none focus:-blue-500 transition-colors appearance-none" required >
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Audio File</label>
                    <div onClick={() => audioInputRef.current?.click()} className="w-full h-24 -2 -dashed -white/10 rounded-[10px] flex flex-col items-center justify-center cursor-pointer hover:-blue-500/50 hover:bg-muted/50 transition-all" >
                      ...
                      <Music className="h-6 w-6 text-muted-foreground/50 mb-2" />
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Cover Art</label>
              ...
              <div className="pt-4 flex gap-4">
                ...
                <button type="submit" disabled={isLoading} className="flex-[2] bg-blue-600 hover:bg-blue-500 text-foreground font-bold uppercase tracking-widest py-4 rounded-[10px] transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2" >
                  {isLoading ? (
          ...
          {step === 3 && (
            <form onSubmit={handleMintSubmit} className="space-y-6 animate-in fade-in slide-in-from-right duration-500 relative z-10">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Listing Price (TON)</label>
                  <input type="number" value={mintData.price} onChange={e => setMintData({...mintData, price: e.target.value})} className="w-full bg-muted/50 -white/10 rounded-[10px] px-4 py-3 text-sm font-bold text-foreground outline-none focus:-blue-500 transition-colors" required />
                </div>
              <div className="space-y-4">
                <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Royalty Splits (%)</label>
                {mintData.royaltySplits.map((split, index) => (
                  <div key={index} className="flex gap-3 items-center mb-2">
                    <input 
                      ...
                      className="flex-1 bg-muted/50 rounded-[10px] px-4 py-3 text-sm font-bold text-foreground outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
                    />
                    <input 
                      type="number" 
                      placeholder="%" 
                      ...
                      className="w-20 bg-muted/50 rounded-[10px] px-4 py-3 text-sm font-bold text-foreground outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
                    />
                  </div>
                ))}
              ...
              <div className="pt-4 flex gap-4">
                ...
                <button type="submit" disabled={isLoading} className="flex-[2] bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-foreground font-bold uppercase tracking-widest py-4 rounded-[10px] transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 shadow-lg shadow-blue-600/20" >
                  {isLoading ? (
                    <>
                      <img src={APP_LOGO} className="w-5 h-5 object-contain animate-[spin_3s_linear_infinite] opacity-80" alt="Loading..." />
                      <span>Minting...</span>
