...
export const AboutSection: React.FC<AboutSectionProps> = ({ profile }) => {
  return (
    <div className="space-y-6 text-white font-sans pb-8">
      {/* Bio Card */}
      <div className="bg-[#101A3B] rounded-2xl p-5 sm:p-6 space-y-4 shadow-sm">
        <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400">
          The Sonic Narrative
        </h4>
        <p className="text-sm leading-relaxed text-slate-200">
          {profile.bio || 'No biography has been added to this profile yet.'}
        </p>

        {/* Location & member specs info */}
        <div className="grid grid-cols-2 gap-4 pt-3 text-xs text-slate-400 bg-white/[0.02] -mx-5 sm:-mx-6 -mb-5 sm:-mb-6 px-5 sm:px-6 py-4 rounded-b-2xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            {profile.socials.website && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="bg-white/5 hover:bg-white/10 p-3.5 rounded-xl flex items-center justify-between group transition-colors cursor-pointer text-slate-200 hover:text-white"
              >
