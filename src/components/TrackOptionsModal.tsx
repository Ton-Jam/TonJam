...
const TrackOptionsModal: React.FC<TrackOptionsModalProps> = ({ track, onClose, onRemove }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-2 animate-in fade-in duration-300">
      ...
      <div className="relative bg-card border border-blue-500/30 w-full max-w-sm rounded-xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
        <div className="p-2 border-b border-blue-500/30 flex items-center gap-2 bg-muted">
          <img src={track.coverUrl || getPlaceholderImage(`track-${track.id}`)} className="w-16 h-16 rounded-md object-cover shadow-lg" alt="" />
          <div>
            <h3 className="text-sm font-bold text-foreground truncate max-w-[200px]">{track.title}</h3>
        ...
        <button onClick={onClose} className="w-full p-2 text-sm font-bold text-muted-foreground hover:text-foreground hover:bg-muted transition-colors border-t border-blue-500/30">
          Cancel
        </button>
      </div>
    </div>
