...
const UserCard: React.FC<UserCardProps> = ({ user, variant = 'portrait' }) => {
  ...
  if (variant === 'compact') {
    return (
      <div 
        onClick={handleClick}
        className="flex items-center gap-3 p-3 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all cursor-pointer group"
      >
  ...
  return (
    <div 
      onClick={handleClick}
      className="flex flex-col items-center text-center p-6 rounded-3xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all cursor-pointer group"
    >
      <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-2 border-white/5 group-hover:border-blue-500/50 transition-all mb-4">
