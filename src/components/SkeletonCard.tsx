...
const SkeletonCard: React.FC<SkeletonCardProps> = ({ className = '', variant = 'default' }) => {
  if (variant === 'row') {
    return (
      <div className={`animate-pulse flex items-center gap-2 p-2 rounded-[10px] bg-muted/50 glass w-full ${className}`}>
        <div className="w-12 h-12 rounded-[5px] bg-muted flex-shrink-0"></div>
  ...
  return (
    <div className={`animate-pulse rounded-[12px] p-3 bg-muted/50 border border-border/50 ${className}`}>
