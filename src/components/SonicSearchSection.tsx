...
import { Search, Loader2, Disc, SearchIcon } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-2 mb-2">
          <ButtonGroupInput 
            ...
            className="flex-1"
            inputClassName="bg-muted/50 border-border p-6 text-sm text-foreground outline-none focus:border-neutral-500/50 transition-all placeholder:text-muted-foreground/50 dark:placeholder:text-neutral-500"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {results.map(track => (
              <div key={track.id} className="flex items-center gap-2 p-2 bg-muted/50 rounded-[10px] hover:bg-muted transition-all cursor-pointer" onClick={() => playTrack(track)}>
                <img src={track.coverUrl || getPlaceholderImage(`track-${track.id}`)} className="w-12 h-12 rounded-[8px] object-cover" alt="" />
