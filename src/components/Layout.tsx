...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex min-h-screen bg-black text-white transition-colors duration-300">
      <a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[100] focus:bg-blue-600 focus:text-white focus:px-4 focus:py-2 focus:rounded-md focus:font-bold">
        Skip to content
      </a>
      ...
      {!isExplore && !isArtistProfile && !isUserProfile && (
        <header className="fixed top-0 left-0 right-0 z-40 bg-black/80 backdrop-blur-md border-b border-white/5 px-4 md:px-6 py-3 flex items-center justify-between lg:left-64 transition-colors duration-300">
          <div className="flex items-center gap-4 flex-1">
            ...
            {!isHome && !isDiscover && (
              <div className={`hidden lg:flex flex-1 relative transition-all duration-300 ${isSearchOpen ? 'max-w-3xl' : 'max-xl'}`} ref={searchRef}>
      ...
      <aside className="hidden lg:flex fixed top-0 left-0 bottom-0 w-64 bg-black border-r border-white/5 flex-col p-6 z-50 overflow-y-auto transition-colors duration-300" aria-label="Main Sidebar">
        ...
        <nav className="flex-1 space-y-2" aria-label="Main Navigation">
          ...
          <div className="mt-8 p-4 rounded-[5px] bg-white/5 border border-white/10 flex items-center justify-between" role="complementary" aria-label="Token Price Info">
      ...
      {/* Main Content Area */}
      <main id="main-content" className={`pb-32 lg:pb-24 lg:ml-64 transition-all w-full ${isExplore ? 'pt-0' : 'pt-16 lg:pt-16'}`}>
