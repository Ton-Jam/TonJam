...
const BidAcceptanceModal: React.FC<BidAcceptanceModalProps> = ({ nft, offer, onClose, onAccept }) => {
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-full max-w-md p-0 overflow-hidden bg-card border border-border rounded-[10px] shadow-2xl">
        ...
        <div className="p-6 relative z-10">
          ...
          <div className="flex items-center gap-4 p-4 bg-muted/50 border border-border/50 rounded-[10px] mb-6">
            <img src={nft.imageUrl || getPlaceholderImage(`nft-${nft.id}`)} className="w-20 h-20 rounded-[10px] object-cover" alt="" />
            <div>
          ...
          <div className="flex flex-col gap-3">
            <button onClick={handleConfirm} disabled={isProcessing} className="w-full py-4 bg-[linear-gradient(90deg,#007AFF_0%,#00C6FF_100%)] hover:opacity-90 text-white rounded-[10px] font-bold text-[11px] uppercase tracking-[0.3em] shadow-2xl shadow-blue-600/20 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed" >
              {isProcessing ? (
