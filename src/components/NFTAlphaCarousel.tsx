...
const NFTAlphaCarousel: React.FC<NFTAlphaCarouselProps> = ({ nfts }) => {
  ...
  return (
    <div className="relative w-full overflow-hidden py-4">
      <div className="flex gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory px-4 sm:px-0">
        {nfts.map((nft, idx) => (
          <motion.div
            key={`alpha-nft-${nft.id}`}
          ...
          >
            ...
            >
              ...
              <div className="absolute top-4 right-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                <button className="p-2 rounded-full bg-black/40 backdrop-blur-md text-white/60 hover:text-blue-400 transition-colors">
              ...
              {/* Liquid decoration */}
              <div className="absolute -bottom-10 -right-10 w-32 h-32 bg-blue-600/20 blur-[60px] rounded-full group-hover:scale-150 transition-transform duration-1000" />
            </div>
          </motion.div>
        ))}
      </div>
