...
interface GlassAiComposeProps {
  isOpen?: boolean
  onClose?: () => void
  onSubmit?: (content: string, model: string, images: string[], webSearch: boolean) => void
}

export default function GlassAiCompose({ isOpen, onClose, onSubmit }: GlassAiComposeProps = {}) {
  ...
  // Click outside to deactivate or close modal
  useEffect(() => {
  ...
  function handleSend() {
    if (!canSend) return
    if (onSubmit) {
      onSubmit(message, activeModel.label, images, webSearch)
    }
    setMessage('')
    setImages([])
  ...
  return (
    <div className={
      isModal 
        ? "fixed inset-0 z-[100] bg-black/75 backdrop-blur-xl flex items-center justify-center p-4" 
        : "relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-[#1A1A19]"
    }>
      {/* Background image */}
      {!isModal && (
        <img
      ...
      >
        ...
        >
          ...
          {isModal && onClose && (
            <button 
              onClick={onClose}
              className="absolute top-4 right-4 z-20 text-white/50 hover:text-white transition-colors cursor-pointer p-1 rounded-full hover:bg-white/5"
            >
              <X size={16} />
            </button>
          )}
