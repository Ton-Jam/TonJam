...
const MiniAudioPlayer: React.FC<MiniAudioPlayerProps> = ({ onOptionsClick }) => {
  const navigate = useNavigate();
  const { currentTrack, isPlaying, togglePlay, progress, closePlayer, setFullPlayerOpen } = useAudio();
  ...
  return (
    <div 
      className="fixed bottom-14 left-0 right-0 z-[45] bg-black border-t border-white/10 px-4 py-2 flex items-center justify-between shadow-2xl h-16 cursor-pointer hover:bg-white/[0.03] transition-all rounded-none"
      onClick={() => setFullPlayerOpen(true)}
    >
      <div className="flex items-center gap-3 w-[65%] cursor-pointer">
        <div className="relative w-11 h-11 rounded-lg overflow-hidden flex-shrink-0 border border-white/5">
      ...
      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
        <button 
          onClick={togglePlay}
          className="w-10 h-10 rounded-full electric-blue-bg flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-lg shadow-blue-500/20"
        >
          <i className={`fas ${isPlaying ? 'fa-pause' : 'fa-play'} text-white text-[10px]`}></i>
        </button>
        
        {/* Track options button immediately next to play button */}
        <button 
