...
const UserCard: React.FC<UserCardProps> = ({ user, variant = 'full' }) => {
  ...
  if (variant === 'portrait') {
    return (
      ...
      >
        ...
        <span className="font-black text-[10px] md:text-xs group-hover:text-blue-400 transition-colors uppercase tracking-tight text-center truncate w-full italic">{user.name}</span>
  ...
  return (
    ...
    >
      ...
      <h3 className="text-sm font-black text-white group-hover:text-blue-400 transition-colors uppercase tracking-tight mb-0.5">{user.name}</h3>
