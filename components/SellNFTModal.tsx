...
const SellNFTModal: React.FC<SellNFTModalProps> = ({ nft, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[400] flex items-center justify-center p-4">
      ...
      <div className="relative glass w-full max-w-md rounded-[2.5rem] border border-blue-500/20 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="p-8">
          ...
          <div className="space-y-6">
            ...
            <div className="flex p-1 bg-white/5 rounded-2xl border border-white/10">
              <button 
                ...
                className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${listingType === 'fixed' ? 'bg-blue-600 text-white shadow-lg' : 'text-white/30 hover:text-white'}`}
              >
                Fixed Price
              </button>
              <button 
                onClick={() => setListingType('auction')}
                className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${listingType === 'auction' ? 'bg-amber-500 text-black shadow-lg' : 'text-white/30 hover:text-white'}`}
              >
                Auction
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className={`w-full bg-black border py-5 pl-14 pr-6 rounded-xl text-2xl font-black text-white outline-none transition-all ${listingType === 'fixed' ? 'focus:border-blue-500 border-white/10' : 'focus:border-amber-500 border-white/10'}`}
                />
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-black border border-white/10 p-3 rounded-xl text-[10px] font-black text-white outline-none focus:border-amber-500 transition-all uppercase"
                    />
                  </div>
                  <div className="space-y-2">
            ...
            <button 
              onClick={handleList}
              disabled={isListing}
              className={`w-full py-4 rounded-xl text-[10px] font-black uppercase tracking-widest text-white shadow-xl active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50 ${listingType === 'fixed' ? 'electric-blue-bg shadow-blue-500/20' : 'bg-amber-500 text-black shadow-amber-500/20'}`}
            >
